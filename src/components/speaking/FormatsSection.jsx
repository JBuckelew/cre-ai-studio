    ...
    <section className="max-w-7xl mx-auto px-6 py-20">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {FORMATS.map((fmt, i) => {
          ...
          return (
            <div key={i} className="rounded-2xl bg-[#1F1F35] border border-[#3F3F5A] p-7 flex flex-col">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="rounded-full border border-[#3F3F5A] px-3 py-1 text-xs text-[#9CA3AF]">
                  {fmt.duration}
                </span>
              </div>
              <h3 className="text-xl font-semibold text-white mb-3">{fmt.title}</h3>
              <p className="text-[#9CA3AF] text-sm leading-relaxed mb-5">{fmt.paragraph}</p>
              <ul className="space-y-2.5 mb-6 flex-1">
                {fmt.bullets.map((b, j) => (
                  <li key={j} className="flex items-start gap-2 text-sm text-[#9CA3AF]">
                    <ArrowRight className="w-4 h-4 text-[#6366F1] mt-0.5 shrink-0" />
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
              <button
                ...
                className="inline-flex items-center gap-1.5 text-sm font-medium text-white hover:text-[#6366F1] transition-colors self-start"
              >
                Request this format
